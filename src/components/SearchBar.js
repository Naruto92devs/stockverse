...
export default function SearchBar({isVisible, setIsvisible}) {
    ...
    return (
        <div className={`fixed top-0 left-0 w-full h-full backdrop-blur-sm bg-black/50 z-20 ${isVisible ? 'visible' : 'hidden'}`}>
            <div ref={searchBarRef} className="xl:w-[40%] lg:w-[50%] sm:w-[65%] w-[94%] h-full lg:pt-[20%] pt-[7vh] flex flex-col gap-y-2 xl:container mx-auto">
                ...
                {/* {noResults && !loading && (
                    ...
                    <ul className="relative flex flex-col bg-primaryBg px-4 rounded-xl min-h-[35vh] lg:max-h-[45vh] max-h-[57vh] overflow-y-auto scrollbar-thin">
                        {/* Loader Start */}
                        <div 
                        className={`${loading ? 'visible' : 'hidden'} absolute top-0 left-0 right-0 bottom-0 w-full flex items-center justify-center backdrop-blur-[2px] bg-black/20`}
                        >
                                ...
                                {results.map((result) => (
                                    ...
                                    >
                                        <div className='flex items-center gap-2'>
                                            ...
                                            <div className="text-md font-sansMedium text-primaryTextColor">{result['1. symbol']}</div>
                                            <div className={`flex items-center gap-1 text-sm font-sansMedium px-2 py-[2px] rounded-full ${parseFloat(1.22) >= 0 ? 'text-buy bg-buy/5' : 'text-sell bg-sell/5'}`}>
                                                +1.22%
                                            </div>
                                        </div>
                                        <div className='flex items-center gap-2'>
                                            <div className='max-md:hidden flex items-center gap-1 text-base font-sansMedium bg-primaryMain/10 border border-primaryMain/5 px-2 pr-3 py-[2px] rounded-full'>
                                                <Image src="/images/level2.png" width={22} height={22} alt='Stockverse Logo' />
                                                Chart
                                            </div>
                                            <div className='flex items-center gap-1 text-base font-sansMedium bg-primaryMain/10 border border-primaryMain/5 px-2 pr-3 py-[2px] rounded-full'>
                        ...
                        {stocks.map((stock) => (
                            <li
                                key={stock['1. symbol']}
                                className="flex py-4 bg-primaryBg items-center justify-between border-b border-black/5"
                                onClick={() => handleSubmitHistory(stock['1. symbol'])}
                            >
                                <div className='flex items-center gap-2'>
                                    <Logo symbol={stock['1. symbol']} alt={stock['2. name']} size={300} className="w-10 h-10 rounded-lg shadow" />
                                    <div className="text-md font-sansMedium text-primaryTextColor">{stock['1. symbol']}</div>
                                    <div className={`flex items-center gap-1 text-sm font-sansMedium px-2 py-[2px] rounded-full ${parseFloat(1.22) >= 0 ? 'text-buy bg-buy/5' : 'text-sell bg-sell/5'}`}>
                                        +1.22%
                                    </div>
                                </div>
