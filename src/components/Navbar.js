            ...
            <div className={`cursor-pointer lg:relative group max-sm:text-[3.6vw] font-medium max-lg:gap-1.5 flex max-lg:p-0 max-lg:flex-col max-lg:items-center items-end px-4 max-xl:px-2 py-2 text-sm max-xl:text-sm text-primaryText lg:hover:bg-primaryText/10 rounded group ${isDropdownVisible ? 'lg:bg-primaryText/10' : ''}`} onClick={toggleDropdown} id="menu-button" aria-expanded="true" aria-haspopup="true" ref={dropdownRef}>
                ...
                <div className={`absolute p-2 max-lg:shadow-none max-lg:w-full max-lg:pb-8 max-lg:left-0 max-lg:bg-mobNavBg max-lg:rounded-2xl max-lg:z-10 transition duration-300 ease-in-out overflow-hidden top-[80%] left-0 mt-2 w-48 bg-background rounded-md shadow-xl max-lg:rounded-none max-lg:fixed max-lg:top-0 max-lg:h-full max-lg:flex max-lg:flex-col max-lg:gap-y-2 max-lg:mt-0 ${isDropdownVisible ? 'opacity-100 visible max-lg:translate-y-[0vh]' : 'opacity-0 invisible max-lg:opacity-100 max-lg:visible max-lg:translate-y-[100vh]'}`}>
                    ...
                    <Link href="/financials">
                        <span className="block px-4 py-2 font-medium max-lg:text-mobNavLink rounded text-base text-primaryText hover:bg-primaryText/10 max-lg:bg-primaryTextHover/20">Financials</span>
                    </Link>
